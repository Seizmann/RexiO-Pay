export const en = {
  common: {
    brand: "RexiO Pay",
    poweredBy: "Powered by RexiO Pay",
    language: "Language",
    english: "English",
    bangla: "বাংলা",
    loading: "Loading…",
    saving: "Saving…",
    cancel: "Cancel",
    close: "Close",
    back: "Back",
    next: "Next",
    skip: "Skip for now",
    done: "Done",
    save: "Save",
    saved: "Saved",
    copy: "Copy",
    copied: "Copied",
    retry: "Try again",
    errorTitle: "Something went wrong",
    errorGeneric: "Something went wrong. Please try again.",
    errorUnauthorized: "Your session has expired. Please sign in again.",
    errorNotConnected: "You are not connected yet.",
    errorPlanLimit:
      "Your current plan has reached this limit. Upgrade your plan to continue.",
    comingSoon: "Coming soon",
    notAvailable:
      "This action needs a backend update that is not ready yet.",
    takaSymbol: "৳",
    currency: "BDT",
    all: "All",
    search: "Search",
    refresh: "Refresh",
    empty: "Nothing here yet.",
    confirm: "Confirm",
    yes: "Yes",
    no: "No",
    signOut: "Sign out",
  },
  errors: {
    plan_limit_exceeded: "Your current plan has reached this limit.",
    domain_not_whitelisted: "That domain is not in your whitelist.",
    idempotency_conflict: "That request conflicts with an earlier one.",
    device_disabled: "That device is disabled.",
    unauthorized: "You are not authorized for this.",
    forbidden: "You do not have permission for this.",
    not_found: "We could not find that.",
    user_not_found: "That person does not have an account yet.",
    invalid_request: "That request is not valid.",
    internal_error: "The server ran into a problem. Try again shortly.",
  },
  status: {
    succeeded: "Succeeded",
    pending: "Pending",
    expired: "Expired",
    canceled: "Canceled",
    active: "Active",
    disabled: "Disabled",
    offline: "Offline",
    verified: "Verified",
    review: "In review",
  },
  auth: {
    signInTitle: "Sign in to RexiO Pay",
    signUpTitle: "Create your RexiO Pay account",
    email: "Email",
    password: "Password",
    signIn: "Sign in",
    signUp: "Create account",
    magicLink: "Email me a sign-in link",
    magicLinkSent: "Check your inbox. We sent you a link to sign in.",
    orUsePassword: "Or sign in with a password",
    havePassword: "Use a password instead",
    noAccount: "No account yet?",
    haveAccount: "Already have an account?",
    passwordRule: "At least 8 characters.",
    passwordShort: "Use at least 8 characters.",
    signedIn: "Signed in.",
  },
  connect: {
    title: "Connect your RexiO Pay API key",
    body: "RexiO Pay has not added merchant self-signup yet, so your account needs to be connected to an API key (rk_live_...) that was issued for your merchant. Paste it here once. The key is stored server-side and is never shown in the browser again.",
    label: "API key",
    placeholder: "rk_live_...",
    connect: "Connect",
    connecting: "Checking…",
    invalid: "The RexiO Pay API rejected that key. Check it and try again.",
    connected: "Connected as",
    disconnect: "Disconnect key",
    howTo: "Your dashboard admin creates keys in the RexiO Pay backend, or use scripts/dev-seed.sql for local testing.",
  },
  dash: {
    title: "Dashboard",
    connected: "API key connected",
    notConnected: "API key not connected",
    connectPrompt: "Connect your API key to use the dashboard.",
    goToConnect: "Connect key",
    onboardingTitle: "Finish setting up",
    onboardingResume: "Continue setup",
    planBadge: "Plan",
    announcement: "Announcement",
    todaySucceeded: "Succeeded today",
    todayPending: "Pending now",
    todayExpired: "Expired today",
    todayVolume: "Volume today",
    matchRate: "Match rate",
    devices: "Devices online",
    noDevices: "No devices paired yet.",
    recentTitle: "Recent sessions",
    recentEmpty: "No sessions today.",
    today: "Today",
  },
  onboard: {
    title: "Set up your account",
    stepKey: "Connect API key",
    stepBranding: "Brand basics",
    stepOtp: "Verify your MFS number",
    stepDevice: "Pair your phone",
    stepDomains: "Whitelist domains",
    stepPlan: "Choose a plan",
    keyDone: "Key connected",
    brandName: "Business name",
    brandEmail: "Support email",
    brandPhone: "Support phone",
    brandPhoneHint: "Customers see this on the payment page.",
    brandColor: "Brand color",
    otpIntro: "To verify that you own this MFS number, send exactly 1 taka from it to the number below. RexiO Pay matches the SMS and creates your first payment profile.",
    otpProvider: "Provider",
    otpAccountType: "Account type",
    otpNumber: "Your MFS number",
    otpStart: "Show payment number",
    otpWaiting: "Waiting for your 1 taka payment...",
    otpVerified: "Number verified. Your first payment profile is ready.",
    otpExpired: "This verification request expired. Start again.",
    otpSendFrom: "Send 1 taka to",
    deviceIntro: "Install the RexiO Pay Engine app on the phone that holds your MFS SIM, then scan this code from the app. The code works for 15 minutes.",
    deviceDownload: "Download the app",
    deviceDownloadNote: "Ask support for the latest APK link for now.",
    deviceName: "Device name",
    deviceCreate: "Generate pairing code",
    deviceRegenerate: "New code",
    deviceScanWith: "Scan with the RexiO Pay Engine app",
    deviceExpires: "Code expires in",
    domainsIntro: "Sessions can only be created for sites on this list. You can change it later.",
    domainsPlaceholder: "example.com",
    domainsAdd: "Add domain",
    planIntro: "Pick a plan now, change it anytime. Payment is manual for now: pay by bKash and the team confirms it.",
    planCurrent: "Current plan",
    finish: "Go to dashboard",
    providerBkash: "bKash",
    providerNagad: "Nagad",
  },
  nav: {
    overview: "Overview",
    transactions: "Transactions",
    profiles: "Payment profiles",
    devices: "Devices",
    inbox: "SMS inbox",
    links: "Payment links",
    keys: "API keys",
    webhooks: "Webhooks",
    branding: "Branding",
    domains: "Domain whitelist",
    settings: "Settings",
  },
  settings: {
    title: "Settings",
    billingNote: "Plan changes are manual for now: pay by bKash and the team confirms the upgrade.",
    currentPlan: "Current plan",
    renews: "Renews",
    sessionsUsed: "Sessions used this period",
    claimWindow: "Claim window (minutes)",
    sessionTtl: "Session TTL (minutes)",
    sessionTtlNote: "Saved, but the server currently applies its own default.",
    amountTolerance: "Amount tolerance (taka)",
    autoLate: "Accept late matches automatically",
    balanceVerification: "Balance verification",
    balanceVerificationHint: "Flags SMS whose reported balance does not match the tracked one.",
    teamTitle: "Team members",
    teamEmail: "Email",
    teamRole: "Role",
    teamInvite: "Invite",
    teamInviteHint: "The person must already have a RexiO Pay account.",
    teamRemove: "Remove",
    languageTitle: "Language",
    savedOk: "Saved",
  },
  modules: {
    overviewTitle: "Overview",
    txTitle: "Transactions",
    txExport: "Export CSV",
    txSession: "Session",
    txAmount: "Amount",
    txCustomer: "Customer",
    txProvider: "Provider",
    txTrxid: "TrxID",
    txStatus: "Status",
    txSource: "Source",
    txDate: "Date",
    txEmpty: "No transactions in this range.",
    profilesTitle: "Payment profiles",
    profilesAdd: "Add profile",
    profilesNumber: "MFS number",
    profilesName: "Display name",
    profilesDevice: "Device",
    profilesSim: "SIM slot",
    profilesBalance: "Tracked balance",
    profilesActions: "Actions",
    profilesPause: "Pause",
    profilesActivate: "Activate",
    profilesEmpty: "No payment profiles yet. Add your first MFS number.",
    profilesDelete: "Delete",
    profileBindNote: "Device binding and balance sync are not available yet.",
    devicesTitle: "Devices",
    devicesAdd: "Pair a device",
    devicesLastBeat: "Last heartbeat",
    devicesLastSync: "Last SMS sync",
    devicesDisable: "Disable",
    devicesRepair: "New pairing code",
    devicesEmpty: "No devices yet.",
    devicesSigFails: "Signature failures",
    linksTitle: "Payment links",
    linksCreate: "Create link",
    linksTitleField: "Title",
    linksAmount: "Amount (blank = customer enters)",
    linksReusable: "Reusable",
    linksCopy: "Copy link",
    linksEmpty: "No payment links yet.",
    linksToggleNote: "Enable/disable is not available yet.",
    linksOpenNote: "Customers open links at the /pay/link page once the backend ships it.",
    keysTitle: "API keys",
    keysCreate: "Create key",
    keysName: "Name",
    keysPrefix: "Prefix",
    keysScopes: "Scopes",
    keysLastUsed: "Last used",
    keysRevoke: "Revoke",
    keysEmpty: "No API keys yet.",
    keysShowOnce: "Copy this key now. It will not be shown again.",
    webhooksTitle: "Webhooks",
    webhooksAdd: "Add endpoint",
    webhooksUrl: "URL",
    webhooksEvents: "Events",
    webhooksLog: "Delivery log",
    webhooksAttempts: "Attempts",
    webhooksResponse: "Response",
    webhooksRedeliver: "Redeliver",
    webhooksEmpty: "No webhook endpoints yet.",
    webhooksStubNote: "Webhook endpoint management needs a backend update and is not available yet. Deliveries are shown read-only.",
    inboxTitle: "SMS inbox",
    inboxFilterAll: "All",
    inboxRaw: "Raw SMS",
    inboxAttach: "Attach",
    inboxDismiss: "Dismiss",
    inboxPaste: "Paste an SMS to process it",
    inboxPastePlaceholder: "Paste the full SMS text here...",
    inboxProcess: "Process",
    inboxEmpty: "No SMS rows.",
    inboxStubNote: "Attach, dismiss, and manual paste need a backend update and are not available yet.",
    brandingTitle: "Branding",
    brandingLogo: "Logo",
    brandingFavicon: "Favicon",
    brandingUpload: "Upload",
    brandingUploading: "Uploading...",
    brandingPreview: "Checkout preview",
    brandingPreviewNote: "Preview with your brand:",
    brandingUploadFailed: "Upload failed. Check the file and try again.",
    domainsTitle: "Domain whitelist",
    domainsAdded: "Added",
    domainsRemove: "Remove",
    domainsEmpty: "No domains whitelisted. Sessions from unknown domains are rejected.",
    settingsTeamEmpty: "No team members yet.",
    planLimitReached: "Plan limit reached",
  },
  admin: {
    title: "Platform admin",
    merchants: "Merchants",
    merchantName: "Name",
    merchantPlan: "Plan",
    merchantStatus: "Status",
    merchantDevices: "Devices",
    merchantPayments: "Payments",
    merchantSessions: "Sessions",
    merchantOwner: "Owner",
    overridePlan: "Override plan",
    disableMerchant: "Disable merchant",
    disabledNote: "This merchant is disabled; their checkout pages show a maintenance state.",
    devicesTitle: "Global device health",
    deviceMerchant: "Merchant",
    smsStats: "SMS parse stats (30 days)",
    smsStatsNote: "Grouped by provider and parse status per day; per-template rates need a backend addition.",
    parsed: "Parsed",
    failed: "Failed",
    auditLogs: "Audit logs",
    auditAction: "Action",
    auditEntity: "Entity",
    auditActor: "Actor",
    auditMerchant: "Merchant",
    announcements: "Announcements",
    announcementTitle: "Title",
    announcementBody: "Body",
    announcementSeverity: "Severity",
    announcementExpires: "Expires (optional)",
    announcementCreate: "Publish",
    announcementDelete: "Delete",
    matchTool: "Manual match",
    matchToolNote: "Cross-tenant manual matching needs a backend endpoint and is not available yet.",
    billingConfirm: "Mark plan as paid/renewed",
    notAdmin: "This account is not a platform admin.",
  },
  checkout: {
    pay: "Pay",
    amount: "Amount",
    timeLeft: "Time left",
    expired: "This payment request has expired.",
    expiredHelp: "Contact the merchant to get a new payment request.",
    canceled: "This payment was canceled.",
    notFound: "We could not find this payment request.",
    notFoundHelp: "Check the link, or contact the merchant.",
    payTo: "Pay to",
    providerPersonal: "Personal",
    providerAgent: "Agent",
    providerMerchant: "Merchant",
    senderNumber: "Your bKash/Nagad number",
    senderNumberHint: "The number you will pay from.",
    senderNumberInvalid: "Enter a valid Bangladeshi mobile number.",
    instructionsTitle: "How to pay",
    step1: "Open your {provider} app",
    step2: "Send Money to {number}",
    step3: "Enter the exact amount {amount}",
    step4: "Confirm with your PIN",
    copyNumber: "Copy number",
    copyAmount: "Copy amount",
    paidQuestion: "Have you paid?",
    paidButton: "I have paid",
    paidInfo:
      "Tap after you have sent the money. Verification usually takes a few seconds.",
    trxidLabel: "Transaction ID (optional)",
    trxidHint: "Find it in your {provider} app history.",
    trxidRequired:
      "This merchant needs the Transaction ID for this payment. Please enter it.",
    reviewTitle: "Verification is taking longer than usual",
    reviewBody:
      "We have your payment confirmation. The merchant will confirm it shortly. You can close this page.",
    successTitle: "Payment received",
    successBody: "Your payment is confirmed. You can close this page now.",
    redirecting: "Taking you back to {merchant}…",
    support: "Need help? Contact {merchant}",
    poweredBy: "Powered by RexiO Pay",
    maintenanceTitle: "Payments are temporarily unavailable",
    maintenanceBody:
      "The merchant has disabled payments for now. Please try again later.",
    fallbackBody:
      "Open your bKash or Nagad app and send the exact amount to the merchant's number. Copy the amount below to avoid mistakes.",
    sessionFailed:
      "We could not load this page. Check your connection and try again.",
  },
  marketing: {
    navProduct: "Product",
    navPricing: "Pricing",
    navDocs: "API docs",
    navFaq: "FAQ",
    navSignIn: "Sign in",
    navGetStarted: "Get started",
    heroEyebrow: "A SpritexAI product",
    heroTitle: "Take bKash and Nagad payments, verified automatically",
    heroBody:
      "RexiO Pay checks every payment SMS from your own phone and tells your site the moment the money arrives. No MFS API contract, no manual checking.",
    heroCta: "Start for free",
    heroSecondary: "Read the API docs",
    howTitle: "How it works",
    howStep1Title: "Create a payment request",
    howStep1Body:
      "Your site calls the RexiO Pay API and the customer lands on a payment page with your branding.",
    howStep2Title: "The customer pays your number",
    howStep2Body:
      "The customer sends money from their bKash or Nagad app to your own MFS number. The money goes straight to you.",
    howStep3Title: "Your phone confirms it",
    howStep3Body:
      "The RexiO Pay Engine app on your phone reads the payment SMS and forwards it, signed, to RexiO Pay.",
    howStep4Title: "Your site hears back instantly",
    howStep4Body:
      "RexiO Pay matches the SMS to the payment request and fires a signed webhook to your site within seconds.",
    pricingTitle: "Plans",
    pricingBody:
      "Flat monthly price. No per-transaction fee. Pay by bKash, confirmed manually while automated billing is on the way.",
    planStarterNote: "For trying things out",
    planProNote: "For growing shops",
    planBusinessNote: "For high volume",
    perMonth: "/month",
    unlimited: "Unlimited",
    profilesLimit: "Payment profiles",
    devicesLimit: "Devices",
    sessionsLimit: "Checkout sessions / month",
    webhooksLimit: "Webhook endpoints",
    teamLimit: "Team members",
    choosePlan: "Get started",
    faqTitle: "Questions people ask",
    faq1Q: "Do you hold my money?",
    faq1A:
      "No. Payments go directly from your customer's MFS account to yours. RexiO Pay never holds, moves, or pools funds. It only reads the payment confirmation SMS and tells your site about it.",
    faq2Q: "Do I need a bKash or Nagad merchant contract?",
    faq2A:
      "No. RexiO Pay works with your existing personal, agent, or merchant MFS account. You are responsible for using it in line with your provider's terms.",
    faq3Q: "What if my phone is offline?",
    faq3A:
      "The app stores unsent SMS and forwards them when you are back online, so payments taken while offline are still verified after reconnecting.",
    faq4Q: "How does RexiO Pay know a payment is real?",
    faq4A:
      "It matches the SMS against the pending payment request using the transaction ID, the sender number, and the time window. A balance check can also flag fabricated SMS before they cause trouble.",
    faq5Q: "Which providers are supported?",
    faq5A:
      "bKash and Nagad today, for personal, agent, and merchant account types. More providers are planned.",
    disclaimerTitle: "About compliance",
    disclaimerBody:
      "RexiO Pay does not hold, move, or pool merchant funds. All money moves directly from the customer to the merchant's own MFS account. RexiO Pay is a verification and automation layer over payments merchants already receive. Every merchant uses their own MFS account and is responsible for compliance with their MFS provider's terms and Bangladesh Bank regulations. Payment aggregation as a licensed PSP/PSO is out of scope. MFS providers may flag or suspend personal accounts that receive high volumes of small payments, so understand your account type's limits.",
    footerDocs: "API docs",
    footerStatus: "Status",
    footerTerms: "Terms",
    footerPrivacy: "Privacy",
    footerOrg: "Organization",
    footerProduct: "Product",
  },
} as const;

/** Structural shape of the dictionary with plain string leaves. */
type DeepStringify<T> = {
  [K in keyof T]: T[K] extends string
    ? string
    : T[K] extends readonly (infer U)[]
      ? U extends string
        ? string[]
        : DeepStringify<U>[]
      : DeepStringify<T[K]>;
};

export type Dict = DeepStringify<typeof en>;
